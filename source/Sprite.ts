...
class Sprite {
 ...
 //------------------------------------------------------------------------------------------Suiveur
 public coller(cible : Sprite, x : number, y: number, frequence : number) : number {
  return setInterval( () => { this.placerSurCible(cible,x,y); }, frequence);
 }

 private placerSurCible(cible : Sprite, x : number, y : number) {
  let distance : number = Math.sqrt(x*x + y*y);
  let angle1 : number = cible.getRotation()/180 * Math.PI;
