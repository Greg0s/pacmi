...
var Sprite = (function () {
    ...
    Sprite.prototype.coller = function (cible, x, y, frequence) {
        var _this = this;
        return setInterval(function () { _this.placerSurCible(cible, x, y); }, frequence);
    };
    Sprite.prototype.placerSurCible = function (cible, x, y) {
        var distance = Math.sqrt(x * x + y * y);
        var angle1 = cible.getRotation() / 180 * Math.PI;
        var angle2 = (y < 0 ? -Math.acos(x / distance) : Math.acos(x / distance));
        x = distance * Math.cos(angle1 + angle2);
        y = distance * Math.sin(angle1 + angle2);
        this.setX(cible.getCentreX() - this.getLargeur() / 2 + x);
        this.setY(cible.getCentreY() - this.getHauteur() / 2 + y);
    };
    return Sprite;
}());
(function (Sprite) {
